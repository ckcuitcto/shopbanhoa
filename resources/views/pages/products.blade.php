...
@section('content')
    ...
    <div class="col-md-12 col-sm-12" style="background-color: #f5f5f5;" id="searchBox">
                ...
                <form action="{{route('getProducts')}}" method="GET">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10 col-sm-10">
                            @if (!empty($oldValue['txtName']))
                                <input type="text" class="form-control" value="{{ $oldValue['txtName'] }}"
                                       name="txtName"
                        ...
                        <div class="col-sm-10">
                            <select class="form-control" name="slPrice">
                                @if (!empty($oldValue['slPrice']))
                                    <option value="0" {{ ($oldValue['slPrice']==0)  ? "selected" : "" }} > Tất cả
                                    </option>
                                    ...
                                    <option value="6" {{ ($oldValue['slPrice']==6 ) ? "selected" : "" }}> Trên
                                        5.000.000đ
                                    </option>
                                @else
                        ...
                        <div class="col-sm-10">
                            <select class="form-control" name="slOrderBy">
                                @if (!empty($oldValue['slOrderBy']))
                                    ...
                                    <option value="1" {{ ($oldValue['slOrderBy']==1)  ? "selected" : "" }}> Sản phẩm
                                        được
                                        mua nhiều
                                    </option>
                                    <option value="2" {{ ($oldValue['slOrderBy']==2)  ? "selected" : "" }}> Sản phẩm
                                        được
                                        xem nhiều
                                    </option>
                                    <option value="3" {{ ($oldValue['slOrderBy']==3)  ? "selected" : "" }}> Giá từ cao
                                        đến
                                        thấp
                                    </option>
                                    <option value="4" {{ ($oldValue['slOrderBy']==4)  ? "selected" : "" }}> Giá từ thấp
                                        đến
                                        cao
                                    </option>
                                @else
                    ...
                    <div class="form-check" align="center">
                        <input class="form-check-input" type="checkbox"
                               {{ (!empty($oldValue['chksoldOut'])) ? "checked" : ""}} name="chksoldOut" id="chksoldOut">
                        <label class="form-check-label" for="chksoldOut">
    ...
    <div class="col-md-12 col-sm-12" style="background-color: #f5f5f5;margin:0">
        @if($products->count() > 0)
            ...
            <br>
            <div class="pagination row justify-content-center">{{ $products->appends(Request::all())->links("pagination::bootstrap-4") }}</div>
            <br>
    @else
